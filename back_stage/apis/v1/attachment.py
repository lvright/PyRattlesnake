...
async def upload_image(image: UploadFile = File(...), token_info: str = Depends(http.token)):
    ...
    if files.filename.split('.')[1] != 'image':
        ...
        # 将文件信息写入数据库并返回文件 id
        file_id = db.execute(attachment.insert().values(
            **{
                'storage_mode': "1",
        ...
        return http.respond(200, True, '上传成功', {
            'url': file_url,
            'fileName': files.filename,
            'id': file_id
        })

@router.post(path='/attachment/uploadFile', summary='上传文件附件')
...
@router.delete(path='/attachment/delete/{ids:path}', summary='删除附件文件')
async def attachment_delete(ids: str, token_info: str = Depends(http.token)):
    ...
    try:
        ...
        for id in ids.split(','):
            # 逐一提交删除
            db.execute(attachment.delete().where(attachment.c.id == id))
            db.commit()
...
@router.post(path='/attachment/saveNetworkImage', summary='保存网络图片')
