  ...
  <div>
    ...
    <a-modal v-model:visible="visible" width="800px" draggable :footer="false">
      ...
      <a-tabs class="tabs">
        ...
        <a-tab-pane key="snake" title="SnakeAdmin">
          <ul class="mine">
            <li
              v-for="icon in skIcon"
              :key="icon"
...
<script setup>
  ...
  import * as maIcons from '@/assets/sk-icons'

  const skIcon = reactive([])
  ...
  for (let icon in maIcons) {
    snakeIcons.push(`sk-icon-${icon}`.toLowerCase())
  }
