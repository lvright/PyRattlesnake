      ...
      <div class="panel-container">
        ...
        <div class="right-panel">
          <div class="right-panel-search">
            ...
            <el-input v-model="queryParams.name" placeholder="应用组名称" clearable></el-input>
        ...
        <el-table-column label="操作" fixed="right" align="right" width="130" v-else>
          <template #default="scope">
            ...
            <el-button
              ...
              v-auth="['system:appGroup:recovery']"
            ...
            <el-button
              ...
              v-auth="['system:appGroup:realDelete']"
              @click="deletes(scope.row.id)"
            >删除</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        dateRange:'',
        api: {
          list: this.$API.appGroup.getList,
          recycleList: this.$API.appGroup.getRecycleList,
        },
        selection: [],
        queryParams: {
          name: undefined,
    ...
    methods: {
      ...
      async batchDel(){
        await this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？`, '提示', {
          ...
          if (this.isRecycle) {
            this.$API.appGroup.realDeletes(ids.join(','))
            this.$refs.table.upData(this.queryParams)
          } else {
            this.$API.appGroup.deletes(ids.join(','))
            this.$refs.table.upData(this.queryParams)
          }
          loading.close();
      ...
      async deletes(id) {
        await this.$confirm(`确定删除该数据吗？`, '提示', {
          ...
          const loading = this.$loading();
          if (this.isRecycle) {
            await this.$API.appGroup.realDeletes(id)
            this.$refs.table.upData(this.queryParams)
          } else {
            await this.$API.appGroup.deletes(id)
            this.$refs.table.upData(this.queryParams)
          }
          loading.close();
          this.$message.success("操作成功")
      ...
      // 恢复数据
      async recovery (id) {
        await this.$API.appGroup.recoverys(id).then(res => {
