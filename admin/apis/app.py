...
async def app_group(
    ...
    if any([status, name]):
        app_group_list = par_type.to_json(db.execute(select(sys_app_group).where(
            sys_app_group.c.name.like('%' + name + '%'),
            sys_app_group.c.status.like('%' + status + '%')).limit(pageSize).offset(offset_page)).all())
    else:
        app_group_list = par_type.to_json(db.execute(select(
            sys_app_group).limit(pageSize).offset(offset_page)).all())
...
async def app_list(token_info: str = Depends(http.token)):
    ...
    app_group_list = [{'name': item['name'], 'id': item['id']}
                      for item in par_type.to_json(db.execute(select(sys_app_group)).all()) if item]
...
@router.delete(path='/app/appGroup/delete/{ids:path}', summary='删除应用分组', tags=['应用模块'])
...
def app_index(
    ...
    if any([status, app_name, app_id]):
        app_list = par_type.to_json(db.execute(select(sys_app).where(and_(
            ...
            sys_app.c.app_id.like('%' + app_id + '%'))).limit(pageSize).offset(offset_page)).all())
    else:
        app_list = par_type.to_json(db.execute(select(sys_app).limit(pageSize).offset(offset_page)).all())
