...
def to_login(form, ip):
    # 查询登录账户和密码
    user_info = par_type.to_json(
        db.execute(select(admin_account).where(admin_account.c.userId == form['username'],
                                               admin_account.c.password == form['password'])).first())
    login_ip = {
        'username': user_info['username'],
        'ip': ip,
        'login_time': now_date_time,
        'status': 0,
        'message': '登陆成功'
    }
    if user_info:
        ...
        db.execute(update(admin_account).where(admin_account.c.id == user_info['id']).values(
