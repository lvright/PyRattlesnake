...
def read_user_info(userId, token_info):
    ...
    if user_info:
        ...
        def get_relation(tabel, data_tabel, ids):
            relation_list = par_type.to_json(db.execute(select(tabel).where(
                tabel.c.userId == user_info['id'])).all())
            data_list = []
            if relation_list:
...
def create_user_data(account, token_info):
    user_info = {
        'username': account['username'], 'status': account['status'], 'phone': account['phone'],
        'nickname': account['nickname'], 'email': account['email'], 'password': '123456',
        'dashboard': account['dashboard'], 'created_at': now_date_time, 'created_by': now_timestamp,
        'remark': account['remark'], 'avatar': account['avatar']
    }
...
@celery.task
def del_user(userId, token_token):
    # 删除指定用户
    try:
