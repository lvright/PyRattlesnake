...
class CRUBLogin(CRUDBase[Admin, Account]):
    ...
    async def go(self, db: AsyncSession, request: Request, form_data: dict) -> dict:
        get_user_sql = select(self.model)\
            .where(self.model.username == form_data['username'],
                   self.model.password == form_data['password'])
        get_user_data = await db.scalars(get_user_sql)
        user_info = jsonable_encoder(get_user_data.first())
        if user_info:
