...
class CRUBAdmin(CRUDBase[Admin, Account]):
    ...
    async def getUserRouters(self, db: AsyncSession, user: dict) -> list:
        ...
        fields = [SystemMenu.id, SystemMenu.title, SystemMenu.type, SystemMenu.hidden, SystemMenu.parent_id,
                  SystemMenu.redirect, SystemMenu.path, SystemMenu.icon, SystemMenu.component, SystemMenu.status,
                  SystemMenu.name]
        sql = select(*fields).where(SystemMenu.status == "1", SystemMenu.hidden == "0") \
            .where(SystemMenu.id == MenuRelation.menu_id, MenuRelation.role_id == RoleRelation.role_id).where(
            user['id'] == RoleRelation.user_id)
        ...
        if routers:
            ...
            for item in routers:
                ...
                item["meta"] = {"icon": item["icon"], "title": item["title"], "type": item["type"],
    ...
    async def updatePassword(self, db: AsyncSession, paw: dict, user_id: int) -> int:
        ...
        if paw["newPassword"] == paw["newPassword_confirmation"]:
            sql = update(self.model).where(self.model.id == user_id).where(
                self.model.password == paw["oldPassword"]).values(password=paw['newPassword'])
