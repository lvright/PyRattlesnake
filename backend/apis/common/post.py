...
async def update_post(id: int, post: PostStructure, db: AsyncSession = Depends(get_db),
...
async def change_status_post(post: ChangeStatus, db: AsyncSession = Depends(get_db),
                             token: str = Depends(check_jwt_token)):
...
async def sort_operation_dept(post: ChangeSort, db: AsyncSession = Depends(get_db),
                              token: str = Depends(check_jwt_token)):
