...
async def change_status_dept(role: ChangeStatus, db: AsyncSession = Depends(get_db),
                             token: str = Depends(check_jwt_token)):
...
async def num_operation_dept(role: ChangeSort, db: AsyncSession = Depends(get_db),
                             token: str = Depends(check_jwt_token)):
...
async def get_dept_by_role(id: int, db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)):
    ...
    result = [{"id": id,
               "depts": [{"id": dept_id, "pivot": {"role_id": id, "dept_id": dept_id}} for item in data for dept_id in
                         str(item["dept_ids"]).split(",") if dept_id]}]
...
async def recycle_role(page: int, pageSize: int,
                       orderBy: Optional[str] = "",
                       orderType: Optional[str] = "",
                       name: Optional[str] = "",
                       code: Optional[str] = "",
