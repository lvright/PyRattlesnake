...
async def delete_annex(
    ...
    for id in annex.ids:
        await getAnnex.tombstone(db, id)
    return resp_200(msg="删除成功")


@router.put(
    path="/system/attachment/recovery",
    response_model=Result,
    summary="恢复被删除的数据"
)
async def recovery_annex(
        annex: Ids,
        db: AsyncSession = Depends(get_db),
        token: str = Depends(check_jwt_token)
):
    for ids in annex.ids:
        await getAnnex.update(db, ids, obj_in={"delete": 0})
    return resp_200(msg="恢复成功")


@router.get(
    path="/system/attachment/recycle",
    response_model=Result,
    summary="获取被删除附件分页列表"
)
async def recycle_annex(
    ...
    result = await getAnnex.getQuery(db, pageSize=pageSize, query_obj=query_obj, delete="1")
