...
async def get_menu_page(
        ...
        title: Optional[str] = "", hidden: Optional[str] = "", maxDate: Optional[str] = "", minDate: Optional[str] = "",
        status: Optional[str] = "", db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)
):
    query_obj = {"name": name, "title": title, "hidden": hidden, "status": status, "maxDate": maxDate, "minDate": minDate}
