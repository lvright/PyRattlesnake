...
async def update_menu(id: int, menu: MenuStructure, db: AsyncSession = Depends(get_db),
...
async def change_status_menu(menu: ChangeStatus, db: AsyncSession = Depends(get_db),
                             token: str = Depends(check_jwt_token)):
...
async def sort_operation_menu(menu: ChangeSort, db: AsyncSession = Depends(get_db),
                              token: str = Depends(check_jwt_token)):
