...
async def get_login_log(
    ...
    return resp_200(data={"items": result, "pageInfo": {"total": total, "currentPage": 1, "totalPage": total_page}})
...
async def upadte_backend_setting(backend_setting: BackendSetting, db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)):
