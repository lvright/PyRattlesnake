...
async def save_user(id: int, user: Account, db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)):
    await getPost.removeRelation(db, user_id=id), getRole.removeRelation(db, user_id=id), getPost.removeRelation(db, user_id=id)
...
async def export_user(page: int = Body(...), pageSize: int = Body(...), db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)):
