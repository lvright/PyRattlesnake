...
async def get_notice_page(
        ...
        title: Optional[str] = "", maxDate: Optional[str] = "", minDate: Optional[str] = "",
        type: Optional[str] = "", db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)
):
    result = await getNotification.getQuery(db, query_obj={"title": title, "type": type, "maxDate": maxDate, "minDate": minDate})
...
async def recycle_notice(
    ...
    result = await getNotification.getQueryReclcle(db, query_obj={"title": title, "type": type, "maxDate": maxDate, "minDate": minDate})
    return resp_200(data={"items": result["data"], "pageInfo": {"total": result["total"], "currentPage": page, "totalPage": result["page_total"]}})

@router.post(path="/system/notice/save", response_model=Result, summary="添加公告")
async def save_notice(notice: SystemNotification, db: AsyncSession = Depends(get_db), token: str = Depends(check_jwt_token)):
